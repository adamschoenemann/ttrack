...
createTask name = do
  dbh <- ask
  r <- liftIO
    $ quickQuery' dbh "SELECT name FROM tasks WHERE name=?" [toSql $ name]
  case r of
    [] -> do
...
startSession task mstart = do
  ...
  liftIO
    $ run
      dbh
      "INSERT INTO sessions (taskId, start, end) VALUES (?, ?, ?)"
...
addSession sess = do
  ...
  r <- liftIO
    $ quickQuery'
      dbh
      "SELECT id FROM sessions WHERE taskId=? AND start=? AND end=?"
    $ sessToSql sess
  case r of
...
getTaskById id = do
  ...
  case r of
    [] -> throwError
      $ NoTaskFound
      $ "No task with id " ++ show id ++ " was found"
    [row @ [id, name]] -> return $ taskFromSql row
    x -> throwError
      $ UnexpectedSqlResult
      $ "unexpected result in getTaskById: " ++ show x
...
getTaskSessions task = do
  ...
  r <- liftIO
    $ quickQuery'
      dbh
      "SELECT * FROM sessions WHERE taskId=?"
      [toSql $ taskId task]
  case r of
